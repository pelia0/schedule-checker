...
Для назви, яку не вдалося зіставити локально, бот може зробити один запит до
Gemini 3.5 Flash-Lite з невідомим текстом і переліком повних назв та скорочень із
аркуша предметів. Модель повертає лише дозволені ідентифікатори предметів або
«NONE»; бот перевіряє формат і бере назву та посилання з аркуша. Посилання,
викладачі й дані Telegram до запиту не додаються. Відповідності Gemini
позначаються у повідомленні; неоднозначна, неповна чи некоректна відповідь
залишається невідомою назвою з попередженням. Результат зберігається локально
без URL і прив'язується до поточного переліку предметів та моделі; після зміни
переліку або моделі бот перевіряє нові назви ще раз, а посилання завжди читається
з найсвіжішого довідника.

Ключ API задається необов'язковою змінною GEMINI_API_KEY у локальному `.env`.
Без ключа або при помилці Gemini анонс працює за звичайними правилами: невідома
назва показується як є, без автоматично підібраного посилання. Google обробляє
