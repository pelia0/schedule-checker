...
викладач біля такого вікна не показується.
...
Частково скорочені назви зіставляються з повними назвами з аркуша предметів:
наприклад, «Облік, оподаткування та страхування в КД» відповідає назві із
закінченням «в комерційній діяльності». Для цього мають збігтися слова назви
та початкові літери скороченої частини. Такий збіг має пріоритет над коротким
скороченням іншого предмета. Якщо підходить кілька предметів або назву не
розпізнано, бот показує початковий текст із попередженням без чужого посилання.

Для назв, які не вдалося зіставити локально, бот надсилає до Gemini 3.5
Flash-Lite невідомі текстові позначки разом із повними назвами та скороченнями з
аркуша предметів. Одне звернення може перевірити кілька назв; після невідомої,
порожньої, некоректної чи невдалої відповіді бот повторює запит до трьох разів
загалом, чекаючи 60 секунд між спробами. У повтор потрапляють лише назви, які ще
не зіставлені. Тайм-аут кожного запиту — до 20 секунд. Після третьої невдалої
спроби зберігається початкова назва й попередження, без вигаданого предмета чи
посилання.

Модель повертає лише дозволені ідентифікатори предметів або «NONE»; бот перевіряє
повноту й коректність відповіді, а назву та посилання бере з аркуша. Посилання,
викладачі й дані Telegram до запиту не додаються. Відповідності Gemini
позначаються у повідомленні. Результати зберігаються локально без URL і
прив'язуються до поточного переліку предметів та моделі; після зміни переліку або
моделі кеш не використовується. Старі позитивні збіги сумісні, але старі негативні
результати не блокують нові три спроби. Посилання завжди читається з актуального
довідника.

Ключ API задається необов'язковою змінною GEMINI_API_KEY у локальному `.env`.
Без ключа або після трьох невдалих спроб Gemini анонс працює за звичайними
правилами: невідома назва показується як є, без автоматично підібраного посилання.
У гіршому разі три тайм-аути з двома паузами можуть затримати анонс приблизно на
три хвилини. Google обробляє
передані назви за умовами тарифу API; для безплатного використання в окремих
регіонах запити можуть використовуватися для вдосконалення продуктів.
