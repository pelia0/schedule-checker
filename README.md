...
# Таблиця замін (CSV для data-driven анонсу)
REPLACEMENTS_CSV_URL=https://docs.google.com/spreadsheets/d/e/YOUR_PUBLISHED_ID/pub?output=csv&gid=0

# HTML-версія таблиці замін для старого Selenium-скріншота
REPLACEMENTS_HTML_URL=https://docs.google.com/spreadsheets/d/e/YOUR_PUBLISHED_ID/pubhtml/sheet?headers=false&gid=0
# SCHEDULE_URL — deprecated fallback для старих .env
SCHEDULE_URL=
...
# Ранковий анонс розкладу (робочі дні)
SCHEDULE_ANNOUNCEMENT_TIME=08:00

# Перевірка днів народження (щодня)
BIRTHDAY_CHECK_TIME=08:00

# Перевірка старої таблиці замін зі скріншотом (робочі дні)
REPLACEMENTS_CHECK_TIME=17:00
# SCHEDULE_CHECK_TIME — deprecated fallback для старих конфігурацій
...
[2026-04-02 20:00:00] 🤖 БОТ ЗАПУЩЕНИЙ. ДІАГНОСТИКА СТАНУ:
   📅 Останній ранковий анонс: Ніколи
   🔎 Остання перевірка замін: Ніколи
   ⏰ Анонс / ДН / заміни: 08:00 / 08:00 / 17:00
...
Ранковий анонс використовує data-driven логіку: він не шукає групу скріншотом і
не зашиває предмети в Python, а читає чотири опубліковані CSV-джерела. Окремий
старий Selenium-чекер замін зі скріншотом збережено для вечірньої перевірки.
...
Усі часи вказуються за Києвом, але це три незалежні задачі:

- Пн–Пт о 08:00 — data-driven анонс розкладу в групу;
- щодня о 08:00 — перевірка днів народження та окремі привітання, якщо є кого вітати;
- Пн–Пт о 17:00 — стара перевірка таблиці замін із відправкою її скріншота,
  якщо знайдено групу.

Для ранкового анонсу і вечірньої перевірки замін зберігаються різні маркери у
bot_state.json, тому одна задача не блокує іншу. Кожна задача виконується не
більше одного разу за дату; маркер записується після завершення перевірки або
успішної відправки відповідного повідомлення. Одночасно запускається лише один
екземпляр бота — другий завершується з помилкою блокування.
